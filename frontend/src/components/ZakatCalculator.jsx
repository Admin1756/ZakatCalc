...
import { ASSET_GROUPS, LIABILITY_FIELDS, CATEGORIES } from '../mock/mockData';
...
export default function ZakatCalculator({ category, currency }) {
  const [step, setStep] = useState(0);
  const [values, setValues] = useState({});
  const totals = useMemo(() => computeTotals(values, currency), [values, currency]);
  ...
  if (!category) {
    return (
      <div className="rounded-2xl border border-dashed border-emerald-900/20 bg-white/60 p-10 text-center">
        <div className="text-emerald-900/60 text-sm">↑ Select your category above to start the guided calculator.</div>
      </div>
    );
  }
      ...
      <div className="px-5 md:px-7 py-5 border-b border-emerald-900/10 bg-gradient-to-r from-emerald-50/60 to-amber-50/30">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-[10px] uppercase tracking-[0.22em] text-amber-700 font-bold">Smart Calculator</div>
              <div className="text-base font-semibold text-emerald-950">{cat?.title}</div>
            </div>
          </div>
          ...
          <div className="zc-fade-in">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="md:col-span-2 rounded-2xl bg-gradient-to-br from-emerald-900 to-emerald-800 text-white p-7">
                ...
                <div className="mt-2 text-sm text-amber-100/80">2.5% of net zakatable wealth above Nisab</div>
              ...
              <div className="rounded-2xl bg-amber-50/70 border border-amber-200/60 p-6">
                ...
                <dl className="mt-3 text-sm space-y-2">
                  ...
                  <div className="flex justify-between text-xs"><dt className="text-emerald-900/60">Silver Nisab</dt><dd className="text-emerald-900/80">{formatMoney(totals.silverNisab, currency)}</dd></div>
                  <div className="flex justify-between text-xs"><dt className="text-emerald-900/60">Gold Nisab</dt><dd className="text-emerald-900/80">{formatMoney(totals.goldNisab, currency)}</dd></div>
                </dl>
              </div>
            </div>
